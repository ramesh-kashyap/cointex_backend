...
"></div>
                    ...
                    <div data-v-6b868a30="" class="page-assets-container tw-min-h-full tw-p-16px">
                        <div data-v-6b868a30="" class="page-assets-header tw-text-white tw-relative">
                            <div data-v-6b868a30="" class="page-assets-header-top">
                                ...
                                <div data-v-6b868a30="" class="tw-text-12px tw-opacity-80" style="color:#020503"> Total Assets (USDT) </div>
                                <div data-v-6b868a30="" class="tw-text-28px tw-font-bold" style="color:#020503"> {{number_format(Auth::user()->available_balance(),2)}} </div>
                                <div data-v-6b868a30=""
                                    class="tw-h-70px tw-flex tw-justify-between tw-items-center tw-px-28px tw-py-16px tw-bg-white tw-bg-opacity-90 tw-rounded-8px">
                                    <div data-v-6b868a30="" class="tw-flex-1 tw-text-center">
                                        <div data-v-6b868a30="" class="tw-text-primary tw-text-18px"> {{currency()}}  {{number_format(Auth::user()->investment()->sum('amount'),2)}} </div>
                                        <div data-v-6b868a30="" class="tw-text-12px tw-text-secondary"> Flexible Funds
                                        </div>
                                    </div>
                                    <div data-v-6b868a30="" class="tw-flex-1 tw-text-center">
                                        <div data-v-6b868a30="" class="tw-text-dark tw-text-18px"> {{currency()}}  {{number_format(Auth::user()->withdrawal()->sum('amount'),2)}} </div>
                            ...
                            <div data-v-6b868a30=""
                                class="tw-pt-28px tw-pb-12px tw-flex tw-justify-between tw-items-center tw-text-dark tw-bg-white">
                                <div data-v-6b868a30="" class="tw-flex-1 tw-flex tw-justify-center tw-items-center" onclick="window.location.href='{{ route('user.fund') }}'">
                                <img src="{{asset('')}}img/invest.png" style="height: 25px;">
                                    <span data-v-6b868a30="" class="tw-pl-8px">Deposit</span></div>
                                <div data-v-6b868a30=""
                                    class="tw-flex-1 tw-flex tw-justify-center tw-items-center van-hairline--left" onclick="window.location.href='{{ route('user.Withdraw') }}'">
                                  
                                    <img src="{{asset('')}}img/withdraw.png" style="height: 25px;">
                                  <span data-v-6b868a30="" class="tw-pl-8px">Withdraw</span>
                                </div>
                            </div>
                        </div>
                        <div class="revenue" data-v-6b868a30="">
                            <div class="tw-mb-12px tw-flex tw-justify-between tw-items-center">
                                ...
                                <div class="tw-flex tw-items-center" id="dateFilterToggle"><span
                                        class="tw-pr-4px tw-text-14px">Last
    ...
    <?php
        $selectedDays = request()->get('days', 7); // Default to 60 days
        $filteredData = array_filter($level_income, function ($value) use ($selectedDays) {
            $createdAt = new DateTime($value['created_at']);
            $thresholdDate = new DateTime();
            $thresholdDate->modify("-$selectedDays days");
    ...
    @foreach ($filteredData as $value)
        <div class="tw-mb-16px tw-flex tw-justify-between tw-items-center">
            <div class="tw-flex tw-items-center">
                <div class="tw-mr-8px tw-w-36px tw-h-36px">
                    <img src="{{ asset('img/6ea225b5411abfcabf56b5cc01a8f6f1.png') }}" alt="" style="width:80%">
                </div>
                <div class="tw-text-16px tw-text-dark">{{ $value['remarks'] }}</div>
            </div>
            <div class="tw-flex-1 tw-text-right">
                <div class="tw-text-14px tw-text-dark">
                    {{ $value['status'] === 'Received' || $value['status'] === 'Active' || $value['status'] === 'Approved' ? 'Received' : $value['status'] }}
                </div>
                <p style="color:green">${{ $value['comm'] }}</p>
                <div class="tw-text-12px tw-text-secondary">{{ date('D, d M Y H:i:s', strtotime($value['created_at'])) }}</div>
            </div>
        </div>
    @endforeach
@endif
                                    ...
                                    <div
                                        ...
                                        <div class="custom-empty van-empty">
                                            <div class="van-empty__image"><img
                                                    src="{{asset('')}}img/empty_data.b69933fd.svg"></div>
            ...
            <div  id="dateFilterPopup" data-v-91b14df4="" class="van-popup van-popup--round van-popup--bottom" style="z-index: 2008; display:none">
                <div data-v-91b14df4="" class="popup_box">
                    <div data-v-91b14df4="" class="scroll">
                        <div data-v-91b14df4="" class="ul">
                            <div data-v-91b14df4="">
                                <div data-v-91b14df4="" class="item" data-date-range="7"> Last 7 days </div>
                            </div>
                            <div data-v-91b14df4="">
                                <div data-v-91b14df4="" class="item" data-date-range="30"> Last 30 days </div>
                            </div>
                            <div data-v-91b14df4="">
                                <div data-v-91b14df4="" class="item" data-date-range="60"> Last 60 days </div>
                            </div>
                            <div data-v-91b14df4="">
                                <div data-v-91b14df4="" class="item" data-date-range="90"> Nearly 90 days </div>
                            </div>
                        </div>
                    </div>
                    <div data-v-91b14df4="" class="esc">Cancel</div>
                </div>
            </div>
            <script>
                document.addEventListener("DOMContentLoaded", () => {
    const filterToggle = document.getElementById("dateFilterToggle");
    const filterPopup = document.getElementById("dateFilterPopup");
    ...
    items.forEach((item) => {
        item.addEventListener("click", (e) => {
            const selectedRange = e.target.getAttribute("data-date-range");

            // Close the popup
            filterPopup.style.display = "none";

            // Load data dynamically (Example with AJAX)
            fetch(`/filter-data?days=${selectedRange}`)
                .then((response) => response.json())
                .then((data) => {
                    // Update the record section dynamically
                    const recordContainer = document.querySelector("#recordSection");
                    recordContainer.innerHTML = data.html; // Assuming the backend sends rendered HTML
                })
    ...
    item.addEventListener('click', (e) => {
        const selectedDays = e.target.getAttribute('data-date-range');

        // Reload page with the selected days

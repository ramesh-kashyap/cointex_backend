...
class Dashboard extends Controller
{
    ...
    public function fatchNews()
    {
        $apiKey = config('services.newsapi.key'); // Store your API key in the config
        $query = 'Apple'; // Example query
        $fromDate = now()->subDays(1)->toDateString();
        $sortBy = 'popularity';
        ...
        if ($response->successful()) {
            ...
            $this->data['totalIncome'] =$articles;
  ...
  //  news api
        ...
        if ($response->successful()) {
          ...
          $filteredArticles = collect($articles)->filter(function ($article) {
              return isset($article['urlToImage'], $article['source']['name'], $article['title']) 
                     && !empty($article['urlToImage']) 
                     && !empty($article['source']['name']) 
                     && !empty($article['title']);
          })->toArray();
      
          $this->data['articles'] = $filteredArticles;
// end news api 
// crypto api

$cryptores = Http::get('https://api.coinlayer.com/list?access_key=c50c52bcc94f1fe4c32357617027c0cd');

if ($cryptores->successful()) {
    $response = $cryptores->json();

    if (isset($response['crypto'])) {
        // Extract the 'crypto' data which contains the cryptocurrency information
        $cryptoData = $response['crypto'];
         
        // Now you can loop through the crypto data if needed
        $this->data['crypto'] = $cryptoData;
    } else {
        // Handle missing 'crypto' key
        $this->data['crypto'] = [];
        Log::error('CoinLayer API: "crypto" key missing in response', ['response' => $response]);
    }
} else {
    // Handle failed HTTP request
    $this->data['crypto'] = [];
    Log::error('CoinLayer API request failed', ['status' => $cryptores->status(), 'body' => $cryptores->body()]);
}
